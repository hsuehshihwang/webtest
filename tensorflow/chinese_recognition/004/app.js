...
async function getCameras() {
    try {
        ...
        logToCanvas(`Error accessing media devices: ${error}`);
    }
}
...
function logToCanvas(message) {
    logMessages.push(message);  // Add new message to log array
    if (logMessages.length > 10) logMessages.shift();  // Limit to 10 messages

    // Clear the canvas
    ctx.clearRect(0, 0, logCanvas.width, logCanvas.height);

    // Draw each log message on canvas
    ctx.font = '14px Arial';
    ctx.fillStyle = 'black';
    logMessages.forEach((msg, index) => {
        ctx.fillText(msg, 10, (index + 1) * 20);  // Display log messages with 20px spacing
    });
}
